...
Horizontile_FOV = 50 # set degrees for horizintile FOV
...
vert_correction = np.array([11.2, -.7, 9.7, -2.2, 8.1, -3.7, 6.6, -5.1, 5.1, -6.6, 3.7, -8.1, 2.2, -9.7, .7, -11.2])/1000 # in mm
...
def run_lidar(folder_path, control_pipe = None, cam_pipe=None):
    global logfile
        ...
        while(state != "exit"):
            ...
            if state == "frame_capture" or state == "continuous_capture":
                ...
                for i in range(12):
                    ...
                    horz_angle = np.copy(((angle_bytes[1]<<8) + angle_bytes[0])/100-90)*np.pi/180 # in radians
                ...
                for i in range(1,pkt_count):
                    ...
                    for i in range(12):
                        ...
                        horz_angle = np.copy(((angle_bytes[1]<<8) + angle_bytes[0])/100 - 90)*np.pi/180
...
if __name__ == "__main__":
    run_lidar("C:\\Users\\lowellm\\Desktop\\Data_Acquisition_Scripts\\Multiprocess_Capture_Program\\test")
