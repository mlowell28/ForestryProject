...
import MultiProcessMasterCamera 
import MultiProcessSlaveCamera
...
def run_camera_controller(folder_path, control_pipe, cam_pipe, fps = 10):
    ...
    global logfile
    logfile = open(folder_path + "\\CamerControllerLog.txt", "w")    
    logfile.write("target FPS: " + str(fps) + '\n')
...
def continuous_capture(folder_name, path = "", fps=10):
    ...
    camera_folder = path + folder_name
    left_camera_folder = camera_folder + "\\left\\"
    right_camera_folder = camera_folder + "\\right\\"
    
    os.mkdir(camera_folder)
    os.mkdir(left_camera_folder)
    os.mkdir(right_camera_folder)
    ...
    mastercam_to_slavecam, slavecam_to_mastercam = multiprocessing.Pipe()
    ...
    mastercam_process = multiprocessing.Process(target=MultiProcessMasterCamera.run_master_camera, args=(mastercam_serial, mastercam_to_camcontroller, left_camera_folder, mastercam_to_slavecam))
    ...
    mastercam_process.join()
    slavecam_process.join()
    return state
...
def frame_capture(folder_name, path = ""):
    ...
    UI_to_camcontroller, camcontroller_to_UI =  multiprocessing.Pipe()            
    mastercam_to_camcontroller, camcontroller_to_mastercam = multiprocessing.Pipe()
    slavecam_to_mastercam, mastercam_to_slavecam = multiprocessing.Pipe()
